...
// Optimization: Use a Set for O(1) collision detection.
// We use a numeric hash (x * tileCount + y) to avoid string serialization overhead.
...
function advanceSnake() {
    ...
    if (head.x === food.x && head.y === food.y) {
        score += 10;
        // Optimization: textContent is faster and more secure than innerHTML for plain text.
...
function collision(head) {
    // Optimization: O(1) lookup instead of O(n) array scan.
...
function createFood() {
    ...
    // Optimization: O(1) lookup to ensure food doesn't spawn on snake.
